...
def lay_thong_tin(uid):
    try:
        ...
        log_success("\n[[[[[[|||||THÔNG TIN TÀI KHOẢN LỬA CHÙA|||||]]]]]")
        log_info(f"UID: {data.get('uid')}")
        log_info(f"Tên Tài Khoản: {basic.get('nickname')}")
        log_info(f"LV: {basic.get('level')}")
        ...
        log_info(f"Đang ở: {basic.get('region')}")
...
"""
        ...
        Write.Print("1. Buff Like Lửa Chùa\n", Colors.green, interval=0.01)
        Write.Print("2. Lấy Thông Tin Tài Khoản Lửa Chùa\n", Colors.cyan, interval=0.01)
        Write.Print("0. Thoát\n", Colors.red, interval=0.01)

        choice = input("\nVUI LÒNG NHẬP: ")

        if choice == "1":
            n = int(input("Bạn Muốn Buff Mấy Tài Khoản ?: "))
            uids = [input(f"Nhập iD Tài Khoản Thứ {i+1}: ") for i in range(n)]
            auto = input("Bạn Có Muốn Treo  Auto Buff Like Không (24h/lần) - (y/n): ").lower()

            if auto == "y":
                while True:
                    for uid in uids:
                        tang_like(uid)
                    log_info("Đang auto buff like... (chờ 24h)")
                    time.sleep(24 * 3600)
            else:
                for uid in uids:
                    tang_like(uid)
